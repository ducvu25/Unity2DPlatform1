...
public class PlayerController : MonoBehaviour
{
    ...
    void Run()
    {
        ...
        rb.linearVelocity = new Vector2(move_x * speedMultiplier, rb.linearVelocity.y);
    }
    void Jump()
    {
        if(Input.GetKeyDown(KeyCode.Space) || Input.GetKeyDown(KeyCode.UpArrow))
        {
            ...
            {
                doubleJump = false;
                rb.linearVelocity = new Vector2(rb.linearVelocity.x, forceJump *0.75f);
                audioController.PlaySound((int)SoundEffect.jump);
            }
        }
    }
    void UpdateAnimation()
    {
        ...
        }else if(rb.linearVelocity.y < -0.1f)
        {
            ...
            if(rb.linearVelocity.y > -30)
                rb.linearVelocity = new Vector2(rb.linearVelocity.x, rb.linearVelocity.y * 1.05f);//Mathf.Max(rb.velocity.y, ))
        }
    ...
    {
        rb.linearVelocity = value; // new Vector2(rb.velocity.x, value);
